...
public class Shit_Move_Lark_1 : MonoBehaviour {
    ...
    private  void _HandleMovement(Vector3 direction)
    {
        ...
        {
            _Slow_Down(ref current_accel, max_accel, stopping_power + Mathf.Abs(friction));
        }
        else
        {
            var angle = Mathf.Abs(Vector3.Angle(primary_dir, direction));
            ...
            else if (angle <= 135.0f && !stop_to_turn)
            {
                if (current_move_speed <= 0.0f)
                    current_move_speed = 5.0f;

                turning = true;
                _Turn(ref current_accel, max_accel, ref current_terminal_vel, turning_terminal_vel, current_move_speed, stopping_power + Mathf.Abs(friction));
            }
            else
            {
